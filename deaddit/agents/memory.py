"""Long-term memory persistence for agent runs.

Visit preparation (intent resolution, length targets, creative
directions, kickoff rendering, initial-message assembly) lives in
:mod:`deaddit.agents.prompts`. This module owns the memory side:
deterministic episode summaries, one-time persona-history backfill, and
the structured memory snapshot a visit prompt renders.
"""
...
# ---------------------------------------------------------------------------
# Structured memory snapshot for visit prompts


@dataclass(frozen=True)
class VisitMemories:
    """Structured long-term memory one visit prompt renders.

    ``backfill`` holds the oldest-first persona-history rows; ``episodes``
    holds the most recent visit notes, oldest-first for rendering.
    """
...
def visit_memories(user_username: str) -> VisitMemories | None:
    """Load a persona's prompt-visible long-term memory; None when empty."""
    backfills = (
    ...
    if not backfills and not episodes:
        return None
    return VisitMemories(
        backfill=tuple(row.content for row in backfills),
        episodes=tuple(row.content for row in reversed(episodes)),
    )
