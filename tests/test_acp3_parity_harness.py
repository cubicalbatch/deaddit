...
def _add_post(
    conn, id, *, user, created_at, model=None, title="t", content="c", sub="python"
):
    conn.execute(
...
def _add_comment(
    conn, id, *, user, created_at, model=None, content="c", post_id=1, parent_id=None
):
    conn.execute(
...
def stats_conn(tmp_path):
    ...
    _add_post(conn, 300, user="A1", created_at="2026-08-18 12:00:00", model="agent:eve")

    # --- in-window rows
    _add_post(conn, 1, user="A1", created_at="2026-08-20 01:00:00", model="agent:eve")
    _add_post(
        conn, 2, user="A2", created_at="2026-08-20 02:00:00", model="agent:malory"
    )
    for i in range(4):
        _add_comment(
            conn,
            10 + i,
            user="A1" if i % 2 else "A2",
            created_at=f"2026-08-20 03:{i:02d}:00",
            model="agent:eve",
        )
    ...
    conn.execute(
        ...
        " VALUES (1, 'create_post', '{\"status\": \"created\"}', ?)",
        (WS,),
    )
    conn.execute(
        "INSERT INTO tool_call (id, name, result, created_at)"
...
def test_criterion_a_outside_high_bound(tmp_path):
    ...
    for i in range(8):
        _add_post(
            ...
            1 + i,
            user="A1",
            created_at=f"2026-08-20 0{i}:00:00",
            model="agent:eve",
        )
    for i in range(12):
        _add_comment(
...
def test_indeterminate_when_empty_window(stats_conn):
    ...
    report = compute_report(
        stats_conn, window_start="2026-07-01 00:00:00", window_end="2026-07-02 00:00:00"
    )
...
def test_window_derivation_from_max_created_at(tmp_path):
    ...
    _add_comment(conn, 1, user="a", created_at="2026-08-20 15:30:00", model="agent:x")
    conn.commit()
...
def sample_conn(tmp_path):
    ...
    for i in range(10):
        _add_post(
            ...
            content=("line one\nline two " + "x" * 200),
            created_at=f"2026-08-20 00:{i:02d}:00",
            model=f"agent:p{i % 2}",
        )
    for i in range(15):
        _add_comment(
            ...
            post_id=1 + i % 10,
            content=f"Comment {i}\nsecond line",
            created_at=f"2026-08-20 01:{i:02d}:00",
            model="agent:c",
        )
    conn.commit()
...
def test_packet_deterministic_same_seed(sample_conn):
    first = build_sample_packet(
        sample_conn, seed=42, min_items=20, window_start=WS, window_end=WE
    )
    second = build_sample_packet(
        sample_conn, seed=42, min_items=20, window_start=WS, window_end=WE
    )
    assert first == second
...
def test_packet_different_seed_different_selection(sample_conn):
    a = build_sample_packet(
        sample_conn, seed=1, min_items=20, window_start=WS, window_end=WE
    )
    b = build_sample_packet(
        sample_conn, seed=2, min_items=20, window_start=WS, window_end=WE
    )
    assert a != b
...
def test_packet_rubric_and_header(sample_conn):
    packet = build_sample_packet(
        sample_conn, seed=7, min_items=20, window_start=WS, window_end=WE
    )
...
def test_packet_snippet_truncation_and_newlines(sample_conn):
    packet = build_sample_packet(
        sample_conn, seed=3, min_items=20, window_start=WS, window_end=WE
    )
