...
| D3 notifications/inbox | LeadD3c | **done** | 99d9e23 | PASS (indep. TesterD3; emission matrix + hourly dedupe (recipient,kind,post_id,actor), keyset cursor stable under inserts, FakeProvider trace proves unread-notice → view_inbox → reply-in-abandoned-thread, failure isolation after fix-loop r1 (PendingRollbackError escaped → id snapshot + rollback-before-log), nightly purge 03:45; 445p/1s + ruff clean) | Notification table via e5d7f9a1c3b9 (head; down d2c4f8a16e90) — PROD NOT YET UPGRADED: apply d2c4f8a16e90 THEN e5d7f9a1c3b9 once under md5-snapshot protocol after burn-in ends, BEFORE AC-P2 worker restart. get_inbox/mark_inbox_read service surface (no HTTP endpoints — decision 5); view_inbox = open-inbox semantics (marks read, returns pre-view unread). kind='mod_action' in schema, no emitter — D4 builds it. Old last_inbox_check_at marker removed (clean cutover). |
