...
| `loop.py` | `run_once` takes the numeric agent ID and optional requested intent; `reserve_persona_run` owns persona selection (random pool minus fixed-bound and running personas, empty scheduled pool ⇒ non-strike backoff); records `AgentRun.intent`; turn loop with budgets (default 30 actions / 300s), failure backoff + 5-strike disable, sets next `next_run_at`. |
